...
const prometheusProviderLabelValue = "gcp"

// RecordGcpAPIMetric records a prometheus metric for GCP API calls.
// * If there is an error then it will increment the APIFailedRequestCount counter vec metric.
// * If the GCP API call is successful then it will record 2 metrics:
...
func RecordGcpAPIMetric(err error, gcpServiceName string, invocationTime time.Time) {
	if err != nil {
		metrics.APIFailedRequestCount.
			WithLabelValues(prometheusProviderLabelValue, gcpServiceName).
			Inc()
		return
	}
	...
	// compute the time taken to complete the GCP service call and record it as a metric
...
// GcpAPIMetricRecorderFn returns a function that can be used to record a prometheus metric for GCP API calls.
...
func GcpAPIMetricRecorderFn(gcpServiceName string, err *error) func() {
	invocationTime := time.Now()
	return func() {
		RecordGcpAPIMetric(*err, gcpServiceName, invocationTime)
	}
}
